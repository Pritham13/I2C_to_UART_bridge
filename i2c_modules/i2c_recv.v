module i2c_slave (
  ...
  ////////////////start detect /////////////// 
  always @(posedge w_start_rst or negedge i_SDA) begin
    if (w_start_rst) r_start_detect <= 1'b0;
    else r_start_detect <= i_SCL;
  end
  always @(posedge i_RST or posedge i_SCL) begin
    if (i_RST) r_start_resetter <= 1'b0;
    else r_start_resetter <= r_start_detect;
  end
  //the START just lasts for one cycle of SCL
  ////////////////////state machine//////////////////// 
  always @(posedge i_RST or negedge i_SCL) begin
    if (i_RST) r_state <= s_IDLE;
    else if (r_start_detect) r_state <= s_DEV_ADDR;
    else if (r_addr_act_bit)//at the 9th cycle and change the state by ACK
        begin
      case (r_state)
        s_IDLE: r_state <= s_IDLE;
        s_DEV_ADDR:
        if (!w_address_detect)  //addr don't match
          ...
          r_state <= s_WRITE;
        s_READ: r_state <= s_READ;
        s_WRITE: r_state <= s_WRITE;  //when the state is write the state 
      endcase
    end  //if don't write and master send a stop,need to jump idle
         //the stop_detect is the next cycle of ACK
    else if (r_stop_detect) r_state <= s_IDLE;
  end
  ////////////////stop detect /////////////// 
  always @(posedge w_stop_rst or posedge i_SDA) begin
    if (w_stop_rst) begin
      r_stop_detect <= 1'b0;
    end else begin
      r_stop_detect <= i_SCL;
  ...
  always @(posedge i_SCL) begin
    if (r_addr_act_bit || o_ack_bit || r_start_detect || (r_state == s_IDLE)) begin
      r_bit_counter <= 4'h0;
    end else begin
